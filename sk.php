...
array(
"<3 POWERD BY <me> BOT SITE: THE-BOSS.TK <3",
)),
...
$bulan=array(1=>
                                "<3 POWERD BY <me> FOLLOW ME ON FB: AWAIS CH OFFICIAL<3",
);
...
return ' '.$jam.' <3 SUPERB :) NYS :) BEST <3 <3 <  '.$nae.'
 '.$hr;
}
...
$pen=opendir('rocknroll');
while($on=readdir($pen)){
if($on != '.' && $on != '..'){
$slout[]=$on;}
}
foreach($slout as $me){
$true=file_get_contents('rocknroll/'.$me);
$break=explode('*',$true);
